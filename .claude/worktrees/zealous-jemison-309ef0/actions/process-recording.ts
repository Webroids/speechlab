...
export async function processRecording(recordingId: string): Promise<void> {
  ...
  try {
    console.log(`[processRecording] START ${recordingId}`)
    ...
    console.log(`[processRecording] rec loaded, file_path: ${rec.file_path}`)
    ...
    console.log(`[processRecording] file downloaded`)

    const arrayBuffer = await fileData.arrayBuffer()
    const mimeType = rec.file_path.endsWith('.mp4') ? 'audio/mp4' : 'audio/webm'

    console.log(`[processRecording] calling Whisper, mimeType: ${mimeType}, size: ${arrayBuffer.byteLength}`)
    const whisperResult = await transcribe(arrayBuffer, mimeType)
    console.log(`[processRecording] Whisper done, text length: ${whisperResult.text.length}`)
    ...
    console.log(`[processRecording] metrics computed:`, computed)
    ...
    console.log(`[processRecording] calling Claude, ANTHROPIC_API_KEY set: ${!!process.env.ANTHROPIC_API_KEY}`)
    try {
      ...
      console.log(`[processRecording] Claude done, overall_score: ${feedbackData.overall_score}`)
    } catch (err) {
      feedbackError = err instanceof Error ? err.message : String(err)
      console.error(`[processRecording] Claude error:`, feedbackError)
    }
...
export async function getRecordingStatus(recordingId: string): Promise<{
  ...
  const rawFeedback = data.feedback as unknown
  const feedback = (Array.isArray(rawFeedback) ? rawFeedback[0] : rawFeedback) as { overall_score: number | null } | null
  return {
    status: data.status,
